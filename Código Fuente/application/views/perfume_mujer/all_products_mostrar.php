<h1>Mujer</h1>
	<em></em>
	<h2><a href="<?= base_url('home/mostrar')?>">Inicio</a><label>/</label>Mujer<label>/</label>Todos los productos</h2>
</div>
</div>
<div class="product">
	<div class="container">
...
<!-- RECORRE POR CADA UNO DE LOS ELEMENTOS DEL ARREGLO PASADO A TRAVÉS DE $DATA -->
<?php foreach ($perfumes as $per): ?>
	<div class="col-md-4 item-grid1 simpleCart_shelfItem">
		<div class=" mid-pop">
			<div class="pro-img">
				<img src="<?= base_url('product_images/'.$per['id'].'.png')?>" class="img-responsive" alt="">
				<div class="zoom-icon ">
					<!-- IMAGEN DEL PRODUCTO -->
					<a class="picture" href="<?= base_url('product_images/'.$per['id'].'.png')?>" rel="title" class="b-link-stripe b-animate-go  thickbox"><i class="glyphicon glyphicon-search icon "></i></a>
					<a href="<?= base_url('perfume/perfume_producto/'.$per['id']); ?>"><i class="glyphicon glyphicon-menu-right icon"></i></a>
				</div>
			</div>
			<div class="mid-1">
				<div class="women">
					<div class="women-top">
						<span><?= $per['marca_nombre'];?></span>
						<h6><a href="#"><?= $per['nombre']; ?></a></h6>
					</div>
					<!-- IMAGEN DEL CARRITO -->
					<div class="img item_add">
						<?php if ($this->session->userdata('logged_in')){?>
							<a href="<?= base_url('pedido/comprar/'.$per['id']); ?>"><img src="<?= base_url('assets/images/ca.png')?>" alt="" width="40" height="40"></a>
						<?php 	}else{ ?>
			...
			<div class="mid-2">
				...
				<p ><em class="item_price">$<?= $per['precio'];?></em></p>
			<?php if ($this->session->userdata('is_admin')){?>
                <a href="<?= base_url('perfume/editar/'.$per['id']); ?>" class="text-primary"><i class="fa fa-fw fa-edit pull-right"></i></a>  
                <a href="<?= base_url('perfume/eliminar/'.$per['id']); ?>" class="text-danger" onClick="return confirm('¿Está seguro que desea eliminar este registro?');"><i class="fa fa-fw fa-trash pull-right"></i></a>
            <?php 	} ?>
          </td>
				<div class="block">
